...
/                   without write.
/
/ Apr 01,'08 R0.06  Added fputc(), fputs(), fprintf() and ().
...
char *results[14]={"OK","NOT READY","NO FILE","NO PATH","INVALID NAME","INVLAID DRIVE","DENIED","EXIST","RW ERROR","WRITE PROTECTED","NOT ENABLED","NO FILESYSTEM","INVALID OBJECT","MKFS ABORTED"};
...
)
{
...
#if _USE_STRFUNC >= 2
	if (chr == '\n') fputc1 ('\r', fil);	/* LF -> CRLF conversion */
#endif
	...
	return bw ? chr : EOF1;		/* Return the resulut */
}
...
)
{
	...
	for (n = 0; *str; str++, n++) {
		if (fputc1(*str, fil) == EOF1) return EOF1;
	}
	return n;
}
...
)
{
	...
	for (cc = res = 0; cc != EOF1; res += cc) {
		...
		if (c == 's') {				/* Type is string */
			cc = fputs1(va_arg(arp, char*), fil);
			continue;
		}
		if (c == 'c') {				/* Type is character */
			cc = fputc1(va_arg(arp, char), fil);
			if (cc != EOF1) cc = 1;
			continue;
		}
		r = 0;
