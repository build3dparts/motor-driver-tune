...
ListBox::ListBox(SSD1289_LCD *lcd,OnEvent proc,font_t *font,int sel_color,int font_color,int back_color,int border_color,int ID):GUIComponent(lcd,proc,0,40,320,145,ID)
{
    int i;
    focolor=font_color;
    bkcolor=back_color;
...
void ListBox::AddLine(char *data)
{    
		int len=strlen(data);
		int i;
...
void ListBox::RemoveLine(int id)
{    
   memset(items[id],' ',32);
	 repaint=true;
}

void ListBox::FreeData(void)
{
    int i;
...
void ListBox::ResetInitialState()
{
    int i;
...
int ListBox::TouchDown(int x, int y,void *data)
{
		if (y>16 && y<208)
		{
			prev_line=cline;
...
int ListBox::TouchUp(int x,int y,void *data)
{    
    return GUI_PASS;
}
